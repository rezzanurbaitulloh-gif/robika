...
export function BossPanel() {
  ...
  if (!status) {
    return (
      <div className="rounded-sm border border-border bg-card/60 p-4 text-sm text-muted-foreground">
        Memuat status boss…
      </div>
    );
  }
  ...
  return (
    <div className="rounded-sm border border-rose-400/30 bg-card/60 p-4">
      ...
      {status.can_attempt ? (
        ...
        <>
          ...
          <button
            ...
            className="mt-3 w-full rounded-sm border border-emerald-400/50 bg-emerald-400/10 px-5 py-2 font-display text-xs uppercase tracking-wider text-emerald-300 transition hover:bg-emerald-400/20 disabled:pointer-events-none disabled:opacity-40"
          >
            <span className="inline-flex items-center gap-1.5">
