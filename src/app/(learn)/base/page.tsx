...
const MENU_TILES: { href: string; label: string; hotkey: string; icon: IconName; tone: string }[] = [
  ...
  { href: "/daily", label: "Misi Harian", hotkey: "Q", icon: "clock", tone: "text-sky-300" },
  { href: "/profile", label: "Profil", hotkey: "I", icon: "user", tone: "text-violet-300" },
];

export default async function BasePage() {
  if (!getFlags().newHome) redirect("/dashboard");
  ...
  const skin =
    (profile?.skin_id ? getSkinItem(profile.skin_id) : undefined) ?? SKIN_ITEMS[0];
  ...
  return (
    <main className="relative mx-auto flex min-h-dvh max-w-3xl flex-col gap-4 px-4 py-4">
      ...
      <nav aria-label="Menu markas" className="grid grid-cols-3 gap-2 sm:grid-cols-6">
        {MENU_TILES.map((tile) => (
          <Link
            key={tile.href}
            href={tile.href}
          className="menu-tile flex flex-col items-center gap-1.5 rounded-md border border-border bg-card/70 px-2 py-3 transition hover:-translate-y-0.5 hover:border-cyan-400/50 hover:bg-card"
          >
